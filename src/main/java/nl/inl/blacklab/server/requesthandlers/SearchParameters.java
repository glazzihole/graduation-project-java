...
public class SearchParameters {
	...
	public static SearchParameters get(SearchManager searchMan, boolean isDocs, String indexName, HttpServletRequest request) {
		...
		{
			// 兼容Dictool框架的分页参数
			String pageNo = ServletUtil.getParameter(request, "pageNumber", "").trim();
			int first = 0;
			try {
				//要-1是因为first从0开始，但pageNo是从1开始的
				first = Integer.parseInt(pageNo)-1 < 0 ? 0 : Integer.parseInt(pageNo) - 1;
			} catch (Exception e) {
				;
			}
