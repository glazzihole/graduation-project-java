...
//        KEY_TO_SENTENCEIDS = (Map<String, String>)ois.readObject();
        if (KEY_TO_SENTENCEIDS == null || KEY_TO_SENTENCEIDS.isEmpty()) {
            // 读取语料
...
//                    case "idobj":
                    ...
                    case "csubj":
                        verbNounRegex = "(VB[A-Z]{0,1})-(NN[A-Z]{0,1})";
                            ...
                            if (CorpusConstant.COPULA_LEMMA_SET.contains(edge.getGovernor().lemma())) {
                                ...
                                for (SemanticGraphEdge semanticGraphEdge : dependency.edgeListSorted()) {
                                    if (semanticGraphEdge.getRelation().toString().equals("nsubj") ||
                                        ...
                                        // 存储到Map中
                                        String key = (edge.getDependent().lemma() + "_JJ_" + subject + "_NN").toLowerCase();
